...
{
    QVector<QPointF> startVector = start.value<QVector<QPointF> >();
    QVector<QPointF> endVector = end.value<QVector<QPointF> >();
...
void XYAnimation::updateCurrentValue (const QVariant &value)
{
    if(state()!=QAbstractAnimation::Stopped){ //workaround
        m_dirty = true;
        QVector<QPointF> vector = value.value<QVector<QPointF> >();
